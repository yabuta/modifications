...
namespace voltdb {
    ...
    public:
        CUDAH CompactingStringStorage();
        CUDAH ~CompactingStringStorage();

        CUDAH boost::shared_ptr<CompactingStringPool> get(size_t size);

        CUDAH boost::shared_ptr<CompactingStringPool> getExact(size_t size);

        CUDAH std::size_t getPoolAllocationSize();
