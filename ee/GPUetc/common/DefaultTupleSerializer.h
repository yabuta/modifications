...
public:
     ...
     */
    CUDAH void serializeTo(TableTuple tuple, ReferenceSerializeOutput *out);

    /**
     * Calculate the maximum size of a serialized tuple based upon the schema of the table/tuple
     */
    CUDAH int getMaxSerializedTupleSize(const TupleSchema *schema);

    virtual CUDAH ~DefaultTupleSerializer() {}
};
}
}
