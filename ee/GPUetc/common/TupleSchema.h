...
inline CUDAH const TupleSchema::ColumnInfo* TupleSchema::getColumnInfo(int columnIndex) const {
    return &reinterpret_cast<const ColumnInfo*>(m_data + (sizeof(uint16_t) * m_uninlinedObjectColumnCount))[columnIndex];
}

inline CUDAH TupleSchema::ColumnInfo* TupleSchema::getColumnInfo(int columnIndex) {
